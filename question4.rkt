...
;;checks if its a let expression in the eval procedure
...
(define (let->combo exp)
  ...
  ;gets the let assignment
  (define (let-ass exp) (cadr exp))
  
  ;gets the let body
  (define (let-body exp) (cddr exp))
  
  ;gets the let expression
  (define (let-exp assignment)
    (if (null? assignment) '()
        (cons (cadr (car assignment)) (let-exp (cdr assignment)))))
...
;;; try:    ((lambda (x)(* x x))3)    (((lambda (x) (lambda (y)(+ x y))) 3) 4)
